...
interface UserOverviewProps {
  ...
  aiInsight: string;
  debitChangePct: number;
  kreditChangePct: number;
  debitDiffAmount: number;
  kreditDiffAmount: number;
  pctOperasional: number;
  pctLayananHosting: number;
  pctLainLain: number;
  pctOperasionalBudget: number;
  pctServerBudget: number;
}
        ...
        <div className="group relative bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-15px_rgba(30,136,229,0.08)] overflow-hidden transition-all hover:-translate-y-1 hover:shadow-[0_20px_50px_-15px_rgba(30,136,229,0.12)]">
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex justify-between items-start">
              ...
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${debitChangePct >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                {debitChangePct >= 0 ? `+${debitChangePct}%` : `${debitChangePct}%`}
              </span>
            </div>
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium mt-1">
                {debitDiffAmount >= 0 ? "↑" : "↓"} {formatRupiah(Math.abs(debitDiffAmount))} vs Bulan Lalu
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="group relative bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-15px_rgba(239,68,68,0.08)] overflow-hidden transition-all hover:-translate-y-1 hover:shadow-[0_20px_50px_-15px_rgba(239,68,68,0.12)]">
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex justify-between items-start">
              ...
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${kreditChangePct >= 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                {kreditChangePct >= 0 ? `+${kreditChangePct}%` : `${kreditChangePct}%`}
              </span>
            </div>
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium mt-1">
                {kreditDiffAmount >= 0 ? "↑" : "↓"} {formatRupiah(Math.abs(kreditDiffAmount))} vs Bulan Lalu
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] flex flex-col items-center">
          ...
          <div className="w-full mt-12 space-y-4">
            <div className="flex items-center justify-between group cursor-default">
              ...
              <span className="text-xs font-black text-slate-900">{pctOperasional}%</span>
            </div>
            <div className="flex items-center justify-between group cursor-default">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-amber-500" />
                <span className="text-xs font-bold text-slate-500 group-hover:text-slate-900 transition-colors">Layanan & Hosting</span>
              </div>
              <span className="text-xs font-black text-slate-900">{pctLayananHosting}%</span>
            </div>
            <div className="flex items-center justify-between group cursor-default">
              <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-black text-slate-900">{pctLainLain}%</span>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-8 flex-grow">
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-sky-200/80">
                    ...
                    <span>{pctOperasionalBudget}% Terpakai</span>
                  </div>
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden border border-white/5">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${pctOperasionalBudget}%` }}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-sky-200/80">
                    ...
                    <span>{pctServerBudget}% Terpakai</span>
                  </div>
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden border border-white/5">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${pctServerBudget}%` }}
